// Interpolação — RASCUNHO com melhorias (Plots + estrutura didática)
// Não incluído em main.typ / site. Comparar com interpolacao.typ.
// Mudanças principais:
// - Plots.jl no lugar de CairoMakie
// - objetivos no início; um único capítulo (=) com seções (==)
// - notação/português limpos; Polynomials.fit → fit após using
// - boxes de ideia-chave em Runge / quadratura / singular
// - código de gráfico mais curto e copiável
...
== Notas deste rascunho (para o autor)

Comparar com `interpolacao.typ`. Alterações intencionais:

+ *Plots.jl* em todos os gráficos (sem CairoMakie).
+ Objetivos + mapa no início; segundo `=` virou `==` (um capítulo só).
+ Boxes de ideia-chave (oscilação, Runge, singular $->$ BEM).
+ Português/typos: matriz, polinômio, partes, Spline1D, índices 0-based vs Julia.
+ Código de gráfico mais curto (`plot`/`scatter`/`plot!`).

Ainda *não* feitos neste rascunho (candidatos na revisão):

+ Unificar exercícios espalhados numa seção só no fim de cada bloco.
+ Trocar `Dierckx.Spline1D` por algo mais moderno se quiser (ou manter).
+ Separar “integração numérica” em capítulo próprio (hoje o arquivo mistura interpolação + quadratura + singular + desafio BEM).
+ TOC automático no site e menu lateral recolhível — chrome do `build-site.mjs`, não deste `.typ`.
